...
BEGIN
	...
	BEGIN
		...
		--В каждом service_id одинаковый код,с расчетом на переменные конретного service_id, + глобальные переменные, которые пишутся в таблицу 
		--для понимания когда пользователь считается покинувшим все сервисы (10 дней простоя должно быть(ни одной подписки в этот лаг))

		--эта переменная берется из таблицы. В таблицу записывается с предыдущей строки клиента. Для первой строки = NULL		
		...
		BEGIN										
				...
				--баланс считается заранее, пишется в следующую строку клиент-сервиса + уже существующий баланс
				SET @balance1 = 0;
				...
				--следущая дата оплаты какого-то сервиса клиента
				...
				--рассчитываем гипотетическую дату завершения подписки с учетом баланса
...
SELECT * FROM #temp; --Итоговая таблица с проставленными выходами по клиентам.
